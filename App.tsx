    ...
    <div className="bg-slate-900 text-slate-100 h-screen font-sans flex flex-col md:flex-row overflow-hidden">
      ...
      <main className="flex-1 flex flex-col p-4 md:p-6 relative min-h-0">
          ...
          <div className="flex items-center justify-between flex-wrap gap-y-2">
             <div className="flex items-center gap-2">
                <button 
                  ...
                  className="md:hidden bg-slate-800 p-2 rounded-lg text-slate-300 hover:bg-slate-700 active:bg-slate-600 transition-all transform active:scale-95"
        ...
        {activeTab === 'list' && showBackToTop && (
            <button
            onClick={scrollToTop}
            className="fixed z-20 bottom-6 right-6 md:right-10 bg-sky-600 hover:bg-sky-500 text-white p-3 rounded-full shadow-lg transition-all duration-300 ease-in-out transform hover:scale-110 active:scale-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-sky-500"
...
const TabButton: React.FC<TabButtonProps> = ({ label, icon, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 transform active:scale-95 ${
        isActive
