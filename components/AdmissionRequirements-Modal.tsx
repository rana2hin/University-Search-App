...
// FIX: Define a type for grounding sources to ensure type safety.
...
const tableHeaders = [
    ...
    { key: 'programName', label: 'Program Name' }, // New column
...
export const AdmissionRequirementsModal: React.FC<AdmissionRequirementsModalProps> = ({ university, onClose }) => {
  ...
  // FIX: Added state to store and display grounding sources from Google Search.
  const [sources, setSources] = useState<GroundingSource[]>([]);

  const fetchAdmissionData = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    // FIX: Reset sources for each new request.
    setSources([]);
    let rawText = '';
    let jsonTextToParse = '';
    try {
      ...
      // FIX: Extract and store grounding chunks as required by the guidelines when using googleSearch.
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-7xl max-h-[90vh] flex flex-col border border-gray-600" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-1 overflow-auto">
          ...
          {/* FIX: Display grounding sources at the bottom of the modal content. */}
          {sources.length > 0 && (
